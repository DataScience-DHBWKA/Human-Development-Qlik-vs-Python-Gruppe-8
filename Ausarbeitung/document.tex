...
\begin{document}
	...
	Auch in Qulik können APIs verwendet werden, aber die Anzahl der APIs ist insbesondere in der Standart Version
	
. Mit den Enrsprechenden Bibliotheken bestehen in Python keine Einschränkungen bezüglich des Dateiformats. Da Python lokal genutzt werden kann, bestehen auch keine Einschränkungen bezüglich Dateigrößen oder Uploadzeiten. Außerdem ist mit Bibliotheken wie z.B. Request auch ein einfacher Zugriff auf Api's möglich.
	
	In dem hier verwendeten Beispiel Human Development wird eine größere Anzahl an Datensätzen benötigt. Qlik bietet eine sehr gute Übersicht über darüber welche Datensätze hochgeladen sind, es gibt sowohl Raster als auc h Listenansicht und verschiedene Sortieroptionen. In Python lässt sich nur durch die einzelnen Variablennamen eine Übersicht erkennen.
	
	In Qlik kann sich über die allgemeine Struktur des Datensatzes in dem Overview Reiter eine gute Übersicht gebildet werden. Für weitere Details kann in den Fields Reiter gewechselt werden, um Informationen über die verteilung der Werte in den einzelnen Spalten zu erhalten. Zudem ist hier in Einblick der ersten 10 Zeilen in Tabellenform zu finden und eine Auflistung der einzelnen Spalten inklusive Datentyp. Durch das Ertellen einer App und das Hinzufügen der Daten zu dieser..., viele weitere Funktionen die ich nicht benutzt habe
	
	In Python lassen sich Beispielsweise mit .head() oder isna() ähnliche Effekte erreichen, aber die Darstellung ist weniger schön
